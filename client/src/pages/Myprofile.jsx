...
import { userImageIcon } from '../assets/images/index.js';
export default function Myprofile() {
    ...
    return (
            ...
            <div className='w-full h-full px-12 py-6 overflow-y-scroll max-md:px-3 max-md:pb-[100px]'>
                ...
                <div id='container1' className='flex gap-4 flex-row max-md:flex-col bg-white border rounded-3xl px-3 py-6 mb-5 relative shadow-xl '>
                    <div className=' w-[300px]  h-[300px] overflow-hidden'>
                        <img src={user?.image || userImageIcon} alt='person image' className='w-full h-full object-cover' />
                    </div>
		...
		V385.92z"/>
                        ...
                        {/* edit button  */}
                        <Link to={'/edite/personal-info'} className='border fill-customgreen w-fit h-fit absolute top-10 right-10 flex gap-3 py-6 px-10 rounded-md'>
                            <svg fill="inherite" width="20px" height="20px" viewBox="0 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <span className=' text-3xl leading-15 font-heading font-normal text-primary '>Edit</span>
                        </Link>
                    </div>
                </div>
                ...
                <div id='container1' className='flex gap-4 flex-col bg-white border rounded-3xl text-black px-12 py-6 mt-20 mb-20 relative shadow-xl '>
                    ...
                    <table className='w-full mt-10'>
                        <tbody className='w-full'>
                            ...
                            <tr className=' border-b'>
                                <td> <p className='px-12 py-4 font-body text-3xl leading-10 '>Height</p></td>
                                <td> <p className='px-12 py-4 font-body text-3xl leading-10 '>{user?.height?.value + " " + user?.height?.unit}</p></td>
                            </tr>
                            <tr className=' border-b'>
                                <td> <p className='px-12 py-4 font-body text-3xl leading-10 '>Weight</p></td>
                                <td> <p className='px-12 py-4 font-body text-3xl leading-10 '>{user?.weight?.value + " " + user?.weight?.unit}</p></td>
                            </tr>
                            <tr className=' border-b'>
                                <td><p className='px-12 py-4 font-body text-3xl leading-10 '>Preferred workouts</p></td>
                                <td className='flex px-12 gap-4 flex-wrap flex-row'>{user?.preferredWorkouts?.map((items, index) => (
                                    <p key={index} className=' font-body text-3xl leading-10 ' >{items},</p>
                                ))}</td>
                            </tr>
                            <tr className=' border-b'>
                                <td><p className='px-12 py-4 font-body text-3xl leading-10 '>Dietary preferences</p></td>
                                <td className='flex px-12 gap-4 flex-wrap flex-row'>{user?.dietaryPreferences?.map((items, index) => (
                                    <p key={index} className=' font-body text-3xl leading-10 ' >{items},</p>
                                ))}</td>
                            </tr>
                            <tr className=' border-b'>
                                <td><p className='px-12 py-4 font-body text-3xl leading-10 '>FitnessGoals</p></td>
                                <td className='flex px-12 gap-4 flex-wrap flex-row'>{user?.fitnessGoals?.map((items, index) => (
                                    <p key={index} className=' font-body text-3xl leading-10 ' >{items},</p>
                                ))}</td>
                            </tr>
                            <tr className=' border-b'>
                                <td><p className='px-12 py-4 font-body text-3xl leading-10 '>ActivityLevel:</p></td>
                                <td><p className='px-12 py-4 font-body text-3xl leading-10 '>{user?.activityLevel}</p></td>
                            </tr>
                            <tr className=''>
                                <td><p className='px-12 py-4 font-body text-3xl leading-10 '>MedicalConditions:</p></td>
                                <td className='flex px-12 gap-4 flex-wrap flex-row'>{user?.medicalConditions?.map((items, index) => (
                                    <p key={index} className=' font-body text-3xl leading-10 ' >{items},</p>
                                ))}</td>
                            </tr>
                        </tbody>
                    ...
                    {/* edit button  */}
                    <Link to={'/edite/health-info'} className='border fill-customgreen w-fit h-fit absolute top-10 right-10 flex gap-3 py-6 px-10 rounded-md'>
                        <svg fill="inherite" width="20px" height="20px" viewBox="0 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg">
                            <path d="M30.133 1.552c-1.090-1.044-2.291-1.573-3.574-1.573-2.006 0-3.47 1.296-3.87 1.693-0.564 0.558-19.786 19.788-19.786 19.788-0.126 0.126-0.217 0.284-0.264 0.456-0.433 1.602-2.605 8.71-2.627 8.782-0.112 0.364-0.012 0.761 0.256 1.029 0.193 0.192 0.45 0.295 0.713 0.295 0.104 0 0.208-0.016 0.31-0.049 0.073-0.024 7.41-2.395 8.618-2.756 0.159-0.048 0.305-0.134 0.423-0.251 0.763-0.754 18.691-18.483 19.881-19.712 1.231-1.268 1.843-2.59 1.819-3.925-0.025-1.319-0.664-2.589-1.901-3.776zM22.37 4.87c0.509 0.123 1.711 0.527 2.938 1.765 1.24 1.251 1.575 2.681 1.638 3.007-3.932 3.912-12.983 12.867-16.551 16.396-0.329-0.767-0.862-1.692-1.719-2.555-1.046-1.054-2.111-1.649-2.932-1.984 3.531-3.532 12.753-12.757 16.625-16.628zM4.387 23.186c0.55 0.146 1.691 0.57 2.854 1.742 0.896 0.904 1.319 1.9 1.509 2.508-1.39 0.447-4.434 1.497-6.367 2.121 0.573-1.886 1.541-4.822 2.004-6.371zM28.763 7.824c-0.041 0.042-0.109 0.11-0.19 0.192-0.316-0.814-0.87-1.86-1.831-2.828-0.981-0.989-1.976-1.572-2.773-1.917 0.068-0.067 0.12-0.12 0.141-0.14 0.114-0.113 1.153-1.106 2.447-1.106 0.745 0 1.477 0.34 2.175 1.010 0.828 0.795 1.256 1.579 1.27 2.331 0.014 0.768-0.404 1.595-1.24 2.458z"></path>
                        </svg>
                        <span className=' text-3xl leading-15 font-heading font-normal text-primary '>Edit</span>
                ...
                {/* settings */}
                <div id='container1' className='flex gap-4 flex-col bg-white border rounded-3xl px-6 py-3 mt-10 mb-10 shadow-xl relative '>
                    <h3 className='font-heading text-5xl font-normal leading-12 text-gray-400'>Settings</h3>
                    ...
                    <div className='mt-10 flex flex-col gap-8 text-black'>
                        <div className='flex flex-row w-full'>
                            ...
                            <div className='flex flex-col gap-4 items-start'>
                                ...
                                <label className=" items-center cursor-pointer px-12  flex gap-4 justify-between w-full">
                                    <span className="ms-3 text-3xl font-medium text-gray-900 ">Sms</span>
                                    <input type="checkbox" hidden value="" className="sr-only peer" />
                                    <div className="relative w-24 h-12 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-10 after:w-10 after:transition-all dark:border-primary peer-checked:bg-customgreen"></div>
                                </label>
                                <label className=" items-center cursor-pointer px-12  flex gap-4 justify-between w-full">
                                    <span className="ms-3 text-3xl font-medium text-gray-900 ">App</span>
                                    <input type="checkbox" hidden value="" className="sr-only peer" />
                                    <div className="relative w-24 h-12 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-10 after:w-10 after:transition-all dark:border-primary peer-checked:bg-customgreen"></div>
                                </label>
                            </div>
