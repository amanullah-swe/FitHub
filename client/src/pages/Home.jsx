...
import {
  RemoveMealFromDailyFitnessAndMealsDataAsync,
  caloriesGoal,
  clearDietAndMealApiMessage,
  fetchDailyFitnessAndMealsDataAsync,
  proteinGoal,
  removeWorkout,
  selectDate,
  selectDietAndMealApiMessage,
  selectDocumentId,
  selectFitness,
  selectTotalnutrients,
  selectWorkouts,
  weight,
} from "../features/fitnesAndDiet/fitnessAndDietSlice.js";
      ...
      <div className="px-10 py-5 flex flex-col  w-full max-md:pb-[60px] max-md:px-4">
        ...
        <div className="w-full flex flex-col border rounded-lg bg-white py-1.3">
          ...
          <div className="w-full flex justify-between max-w-[800px] self-center items-center py-3 bg-customgreen rounded-lg text-white">
            ...
            <button
              ...
              onClick={() => setDays((prev) => (days < 0 ? 0 : prev - 1))}
            >
              &gt;
            </button>
          </div>
          <div className="flex w-full justify-evenly mt-10 h-48 max-md:h-32">
            <ProgressBar
              value={Math.floor(totalnutrients.protein)}
              totalvalue={setProtien}
              title="Protien"
            />
            <ProgressBar
              value={Math.floor(totalnutrients.calories)}
              totalvalue={setcalories}
              title="calories"
            />
          </div>
        </div>
        <div className=" w-full overflow-y-scroll">
          <FoodList
            ...
            removebutton={date == getPreviousDate()}
            days={days}
          ...
          />
          <Workoutlist
            handleRemoveButton={handleRemoveWorkout}
            removebutton={date == getPreviousDate()}
            data={workouts}
          />
...
const ProgressBar = ({ value, totalvalue, title }) => {
  ...
  return (
    <div
      ...
      aria-valuemin="0"
      aria-valuemax="100"
      style={{ "--value": persentage }}
    >
      <div className="progress-bar__fill"></div>
      <div className=" absolute top-[50%] progress-percentage">
        {value}/{totalvalue}
      </div>
      <p>{title}</p>
    </div>
